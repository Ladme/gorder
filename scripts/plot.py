...
# Run using `uv`: `uv run -q plot.py INPUT_ORDERMAP` or `uv run -q plot.py INPUT_ORDERMAP OUTPUT_PNG`.
# /// script
...
def main():
    ...
    try:
        ...
        plt.contourf(xi, yi, zi.T, levels=np.linspace(zmin or np.nanmin(z), zmax or np.nanmax(z), 500), cmap=cm, norm=Normalize(vmin=zmin or np.nanmin(z), vmax=zmax or np.nanmax(z)))
        cbar = plt.colorbar(ticks=np.arange(zmin or np.nanmin(z), (zmax or np.nanmax(z)) + (zstep[0] if zstep else (zmax-zmin)/10 if zmax and zmin else 1), zstep[0] if zstep else (zmax-zmin)/10 if zmax and zmin else 1))
        cbar.set_label(zr)
