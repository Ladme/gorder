...
def test_cg_order_basic_yaml():
    for n_threads in [1, 2, 3, 4, 8, 32]:
        ...
        try:
            assert diff_files_ignore_first(temp_file_path, "../tests/files/cg_order_basic.yaml", 1), "Files do not match!"
        finally:
...
def test_all_outputs():
    ...
    try:
        assert diff_files_ignore_first(yaml_path, "../tests/files/aa_order_basic.yaml", 1), "Files do not match!"
        assert diff_files_ignore_first(tab_path, "../tests/files/aa_order_basic.tab", 1), "Files do not match!"
        assert diff_files_ignore_first(csv_path, "../tests/files/aa_order_basic.csv", 0), "Files do not match!"
        assert diff_files_ignore_first(f"{dir_path}/order_POPC.xvg", "../tests/files/aa_order_basic_POPC.xvg", 1), "Files do not match!"
        assert diff_files_ignore_first(f"{dir_path}/order_POPE.xvg", "../tests/files/aa_order_basic_POPE.xvg", 1), "Files do not match!"
        assert diff_files_ignore_first(f"{dir_path}/order_POPG.xvg", "../tests/files/aa_order_basic_POPG.xvg", 1), "Files do not match!"
    finally:
...
def test_leaflets():
    manual_dict = read_leaflets_yaml("../tests/files/inputs/leaflets_files/cg_every.yaml")
...
def test_scrambling_leaflets():
    ...
    ndxs = [["../tests/files/scrambling/ndx/leaflets_frame_000.ndx"],
            [f"../tests/files/scrambling/ndx/leaflets_frame_{x:03d}.ndx" for x in range(0, 101)],
            [f"../tests/files/scrambling/ndx/leaflets_frame_{x:03d}.ndx" for x in range(0, 101, 10)]]

    for (j, freq) in enumerate([gorder.Frequency.once(), gorder.Frequency.every(1), gorder.Frequency.every(10)]):
        for (i, leaflets) in enumerate([
            gorder.leaflets.GlobalClassification("@membrane", "name PO4", frequency = freq),
            gorder.leaflets.LocalClassification("@membrane", "name PO4", radius = 3.0, frequency = freq),
            gorder.leaflets.IndividualClassification("name PO4", "name C4A C4B", frequency = freq),
            gorder.leaflets.ManualClassification(f"../tests/files/scrambling/leaflets_{manual_classification[j]}", frequency = freq),
            gorder.leaflets.ManualClassification(dicts[j], frequency = freq),
            gorder.leaflets.NdxClassification(ndxs[j], "name PO4", "Upper", "Lower", frequency = freq),
        ]):
            ...
            try:
                assert diff_files_ignore_first(temp_file_path, f"../tests/files/scrambling/{files[i][j]}", 1), "Files do not match!"
            finally:
                shutil.rmtree(temp_file_path, ignore_errors=True)
...
def test_x_normal():
    ...
    try:
        assert diff_files_ignore_first(temp_file_path, "../tests/files/aa_order_leaflets.yaml", 1), "Files do not match!"
    finally:
        shutil.rmtree(temp_file_path, ignore_errors=True)
...
def test_dynamic_normals():
    ...
    try:
        assert diff_files_ignore_first(temp_file_path, "../tests/files/cg_order_vesicle.yaml", 1), "Files do not match!"
    finally:
        shutil.rmtree(temp_file_path, ignore_errors=True)

def test_dynamic_normals_leaflets():
    manual_dict = read_leaflets_yaml("../tests/files/inputs/leaflets_files/pcpepg_every.yaml")

    for leaflets in [
        gorder.leaflets.GlobalClassification("@membrane", "name P", membrane_normal = "z"), 
        gorder.leaflets.LocalClassification("@membrane", "name P", radius = 2.5, membrane_normal = "z"), 
        gorder.leaflets.IndividualClassification("name P", "name C218 C316", membrane_normal = "z"),
        gorder.leaflets.ManualClassification("../tests/files/inputs/leaflets_files/pcpepg_every.yaml"),
        gorder.leaflets.ManualClassification(manual_dict),
        gorder.leaflets.NdxClassification(ndx = ["../tests/files/ndx/pcpepg_leaflets.ndx"] * 51, heads = "name P", upper_leaflet = "Upper", lower_leaflet = "Lower")
    ]:
        ...
        try:
            assert diff_files_ignore_first(temp_file_path, "../tests/files/aa_order_leaflets_dynamic.yaml", 1), "Files do not match!"
        finally:
            shutil.rmtree(temp_file_path, ignore_errors=True)
...
def test_begin_end_step():
    ...
    try:
        assert diff_files_ignore_first(temp_file_path, "../tests/files/aa_order_begin_end_step.yaml", 1), "Files do not match!"
    finally:
        shutil.rmtree(temp_file_path, ignore_errors=True)
...
def test_min_samples():
    ...
    try:
        assert diff_files_ignore_first(temp_file_path, "../tests/files/aa_order_limit.yaml", 1), "Files do not match!"
    finally:
        shutil.rmtree(temp_file_path, ignore_errors=True)
...
def test_estimate_error():
    ...
    try:
        assert diff_files_ignore_first(temp_file_path, "../tests/files/cg_order_error.yaml", 1), "Files do not match!"
    finally:
        shutil.rmtree(temp_file_path, ignore_errors=True)
...
def test_convergence():
    ...
    analysis = gorder.Analysis(
        ...
        trajectory = "../tests/files/cg.xtc",
        analysis_type = gorder.analysis_types.CGOrder("@membrane"),
        estimate_error = gorder.estimate_error.EstimateError(output_convergence = temp_file_path),
        silent = True,
        overwrite = True,
    )
    ...
    try:
        assert diff_files_ignore_first(temp_file_path, "../tests/files/cg_order_convergence.xvg", 1), "Files do not match!"
    finally:
        shutil.rmtree(temp_file_path, ignore_errors=True)
...
def test_geometry_cuboid():
    ...
    try:
        assert diff_files_ignore_first(temp_file_path, "../tests/files/cg_order_cuboid_square.yaml", 1), "Files do not match!"
    finally:
        shutil.rmtree(temp_file_path, ignore_errors=True)
...
def test_geometry_cylinder():
    ...
    analysis = gorder.Analysis(
        structure = "../tests/files/pcpepg.tpr",
        trajectory = "../tests/files/pcpepg.xtc",
        analysis_type = gorder.analysis_types.AAOrder("resname POPC and name C22 C24 C218", "@membrane and element name hydrogen"),
        geometry = gorder.geometry.Cylinder(reference = [8, 2, 0], radius = 2.5, orientation = "z"),
        output_yaml = temp_file_path,
        silent = True,
        overwrite = True,
    )
...
def test_geometry_sphere():
    ...
    try:
        assert diff_files_ignore_first(temp_file_path, "../tests/files/cg_order_sphere.yaml", 1), "Files do not match!"
    finally:
        shutil.rmtree(temp_file_path, ignore_errors=True)
...
def test_ignore_pbc():
    ...
    try:
        assert diff_files_ignore_first(temp_file_path, "../tests/files/aa_order_leaflets_nopbc.yaml", 1), "Files do not match!"
    finally:
        shutil.rmtree(temp_file_path, ignore_errors=True)
...
def test_ordermaps_basic():
    ...
    try:
        for file in expected_file_names:
            assert diff_files_ignore_first(f"{dir_path}/POPC/{file}", f"../tests/files/ordermaps_cg/{file}", 2), "Files do not match!"

        assert diff_files_ignore_first(f"{dir_path}/ordermap_average_full.dat", "../tests/files/ordermaps_cg/ordermap_average_full.dat", 2), "Files do not match!"
        assert diff_files_ignore_first(f"{dir_path}/plot.py", "../scripts/plot.py", 0), "Files do not match!"
...
def test_ordermaps_leaflets_nopbc_manual_everything():
    ...
    analysis = gorder.Analysis(
        ...
        ordermap = gorder.ordermap.OrderMap(dir_path, bin_size = [0.1, 4.0], min_samples = 5, dim = [[0, 9], [0, 8]], plane = "xy"),
        handle_pbc = False,
        silent = True,
        overwrite = True,
    )
    ...
    try:
        for file in expected_file_names:
            assert diff_files_ignore_first(f"{dir_path}/POPC/{file}", f"../tests/files/ordermaps_nopbc/{file}", 2), "Files do not match!"

        assert diff_files_ignore_first(f"{dir_path}/ordermap_average_full.dat", "../tests/files/ordermaps_nopbc/ordermap_average_full.dat", 2), "Files do not match!"
        assert diff_files_ignore_first(f"{dir_path}/plot.py", "../scripts/plot.py", 0), "Files do not match!"
