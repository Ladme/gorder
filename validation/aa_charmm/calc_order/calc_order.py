...
# dependencies = ["numpy<2", "mdanalysis>=2.8.0"]
# ///
...
def main():
    ordPars = parse_op_input("mappingPOPCcharmm.txt", "POPC")

    read_trajs_calc_OPs(ordPars, "../files/system.tpr", "../files/traj_whole.xtc")

    with open("order.dat", "w") as outfile:
        line1="Atom     Average OP     OP stem"+'\n'
        outfile.write(line1)

        data = {}

        for i,op in enumerate(ordPars):
